...
# Adding mean to values when 0s in data
if(any(dat[,-1]==0,na.rm=TRUE)){
  if(length(unique(which(dat==0, arr.ind=TRUE)[,2]))>1){
    dat[,unique(which(dat==0, arr.ind=TRUE)[,2])] <- apply(dat[,unique(which(dat==0, arr.ind=TRUE)[,2]),drop=FALSE], 2, function(x){x+(10*mean(x, na.rm=T))/100})
  } else{
    dat[,unique(which(dat==0, arr.ind=TRUE)[,2])] <- dat[,unique(which(dat==0, arr.ind=TRUE)[,2])] + (10*mean(dat[,unique(which(dat==0, arr.ind=TRUE)[,2])], na.rm=T))/100
  }
}

# for global dividing by mean per dataset to get a similar scale (for convergence)
if(ncol(dat)>2){
  for (col in 2:ncol(dat)) {
    dat[,col] <- sapply(dat[,col],function(x) {x/mean(dat[,col], na.rm=T)})
...
ptm <- proc.time()
jara.mod <- jags(jags.data, inits, parameters, "jara.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb)
...
##NP
Rhat <- round(as.data.frame(as.matrix(jara.mod$BUGSoutput$summary))$Rhat,2)
if(any(na.omit(Rhat>1.1))) {jara.mod <- autojags_NP(jara.mod, n.iter=60000, n.thin = 6, n.update=6, Rhat=1.1)}
Rhat <- round(as.data.frame(as.matrix(jara.mod$BUGSoutput$summary))$Rhat,2)
if(any(na.omit(Rhat>1.1))) {
  repeat{
    Rhat <- round(as.data.frame(as.matrix(jara.mod$BUGSoutput$summary))$Rhat,2)
    if(any(na.omit(Rhat>1.1))) {
      cat(red("gelman test fail"))
      print(paste0('Convergence error : ', length(Rhat[which(Rhat>=1.1)]), ' parameters'))
      nber_update <- as.numeric(dlgInput(paste0("Gelman test FAIL\nConvergence error. Update how many times?"))$res)
      if(nber_update==0){
        stop()
        # } else {jara.mod <- autojags_NP(jara.mod, n.iter=60, n.thin = 1, n.update=nber_update)}
      } else {jara.mod <- autojags_NP(jara.mod, n.iter=60000, n.thin = 6, n.update=nber_update)}
    } else{break}
  }
}
...
# boxplot(Resids,add=TRUE,at=c(years),xaxt="n",col=grey(0.8,0.5),notch=FALSE,outline = FALSE) ##NP
# abline(h=0,lty=2)
...
# # den = density(change,adjust=2) ##NP
# # x1 = den$x ##NP
# # y1 = den$y ##NP
# 
# 
# ##NP
# try(logspline_change <- logspline(change, lbound=min(change)), silent = TRUE) # NP
# # logspline_change <- logspline(change, lbound=-100) # NP
# if(exists('logspline_change')){
# fit <- logspline_change # NP
# n=2000 # NP
# # u1 <- qlogspline(0.01, fit) # NP
# # u2 <- qlogspline(0.99, fit) # NP
# # u3 <- 1.1 * u1 - 0.1 * u2 # NP
# u3<-min(change)
# # u4 <- 1.1 * u2 - 0.1 * u1 # NP
# u4<-max(change)
# x1 <- (0:(n - 1))/(n - 1) * (u4 - u3) + u3 # NP
# y1 <- dlogspline(x1, fit) # NP
# } else{
#   den = density(change,adjust=2) ##NP
#   x1 = den$x ##NP
#   y1 = den$y ##NP
...
# summary_change <- data.frame('median' = mu.change, 'CI2.5' = quantile(change,0.025), 'CI97.5' = quantile(change,0.975), row.names = NULL)
# write.table(summary_change,file=paste0(output.dir, "/summary_change.txt"))
# 
# 
# sign=""
# if(mu.change>0) sign="+"
...
# # plot(x1,y1,type="n",xlim=c(-100,-95),ylim=c(0,max(y1*1.05)),ylab=if(exists('logspline_change')){"Logspline density"}else{"Density"},xlab="",cex.main=0.9,frame=T,xaxt="n",yaxt="n",xaxs="i",yaxs="i")
# 
# 
# maxy = max(y1*1.08)
...
# ##NP
# # if(exists('logspline_change')){
#   polygon(c(x1,rev(x1)),c(y1,rep(0,length(y1))),col="grey") ## NP # polygon for logspline
# # } else{
# #   polygon(c(x1,rev(x1)),c(y1,rep(0,length(y1))),col="grey") ## NP # previous polygon for density
# # }
# ##NP
#   
...
#                  paste0("VU (",VU,"%)"),paste0("NT (",NT,"%)"), paste0("LC (",LC,"%)")),col=1,pt.bg=c("#D81E05","#FC7F3F","#F9E814","#CCE226","#60C659"),pt.cex=1.5,pch=22,bg="white",cex=1.24)
...
# jara = list(settings=Settings,data=jags.data,parameters=pars,trends=trends,abundance=abundance.est,perc.risk=data.frame(CR=CR,EN=EN,VU=VU,NT=NT,LC=LC) ,status=status)
# save(jara,file=paste0(output.dir,"/jara.",assessment,".rdata"))

out1 = data.frame(pos=posteriors$Ntot) ## add length year  ##NP
save(out1,file=paste0("./Outputs/", assessment, '/', assessment, "_Ntot.rdata")) ##NP
